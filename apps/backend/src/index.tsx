...
app.post(
  ...
  async (c) => {
    ...
    const message = `Brainwave.ai started as a side project a few months ago when I built it as a hackathon project.
    <br></br>
   ...
   <br></br><br></br>
    So, it's time to make this good. My vision is to make Brainwave.ai the best memory tool on the internet.
    `;

    try {
      await db.insert(waitlist).values({ email });
      await resend.emails.send({
        from: "Dhravya From Brainwave.ai <waitlist@m.Brainwave.ai.com>",
        to: email,
        subject: "You're in the waitlist - A personal note from Dhravya",
        html: `<p>Hi. I'm Dhravya. I'm building Brainwave.ai to help people remember everything.<br></br> ${message} <br></br><br></br>I'll be in touch when we launch! Till then, just reply to this email if you wanna talk :)<br></br>If you want to follow me on X, here's my handle: <a href='https://x.com/dhravyashah'>@dhravyashah</a><br></br><br></br>- Dhravya</p>`,
      });
    } catch (e) {
      console.error(e);
